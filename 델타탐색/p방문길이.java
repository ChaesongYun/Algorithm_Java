...
public class Solution{
    ...
    public static boolean checkLine(int curX, int curY, int nextX, int nextY){
        for(move item: moveList){
            if(item.fromX == curX && item.fromY == curY && item.toX == nextX && item.toY == nextY) return false;
            if(item.fromX == nextX && item.fromY == nextY && item.toX == curX && item.toY == curY) return false;
        }
        return true;
    }
}
